...
| NMEA Status UI | Top-Bar Status-Chip und Uebersicht | Demo, Live, stockende Daten und getrennte Quelle werden ehrlich angezeigt; Tap oeffnet Datenquellen-Uebersicht mit Router- und PGN-Diagnosepfaden | JVM status model + Compile + UI/manual |
...
| Support | Diagnose-Share starten | Ohne Navigator/Fleet wird kein Bericht erzeugt und kein Sharesheet geoeffnet; mit Entitlement ist Consent-Dialog sichtbar, cache-basierte JSON wird per FileProvider/Sharesheet geteilt, Router-Host/MMSI/Route/MOB sind standardmaessig redigiert und es gibt keinen automatischen Upload oder Backend-Triage | JVM policy/runtime gate + UI/manual share |
...
| Chart Source UI | Kartenquellen-Pille im Normal- und Fullscreen-Chart | Provider, Status, Attribution, Import/Premium/License-Badges und Safety-Hinweis bleiben sichtbar; C-Map/S-63 wirken nicht renderbar/kaufbar | JVM source model + Compile + UI/manual |
...
| Premium Pack Install Status | Lokale first-party MBTiles-Datei | Canonical file names werden erkannt; UI zeigt gekauft/nicht installiert/installiert statt Nutzbarkeit zu behaupten | JVM package discovery + UI manual |
...
| Runtime Entitlement Gate | Premium-Widgets hinzufuegen und bestehende Premium-Widgets nach Downgrade/Expiry rendern | Gesperrte Widgets werden vor Erstellung blockiert; bestehende Premium-Widgets bleiben verschiebbar/loeschbar, zeigen aber nur einen Lock-Placeholder und fuehren keine Premium-Widget-Renderlogik aus | JVM runtime gate + UI manual |
...
- Vorhanden: JVM-Tests fuer `GeoCalc`, NMEA-Status-Summary, Boot-Autostart-Policy, Autopilot-Safety-Gate, Backup-Privacy, Chart-Provider-Verfuegbarkeit, Chart-Source-UI-State, Free-Raster-Provider-Vertraege, MBTiles/GeoPackage-Sideload-Dateivertraege, S-57-Renderer-Skeleton/SCAMIN/SOUNDG, oeSENC-Nichtunterstuetzung, Premium-Chart-Pack-Status und lokale Premium-Pack-Datei-Discovery, persistierte SeaChart-Provider-Normalisierung inklusive JSON-Parsing, Hazard-Depth-Filter, Safety-Contour-Policy, Entitlement-Policy, Feature-Access-Policy, Runtime-Entitlement-Gate fuer Widget-Erstellung, bestehende Premium-Widget-Nutzung nach Downgrade/Expiry und Support-Diagnose-Featurezugriff, Billing-Katalog, Play-Purchase-Mapping, Billing-Validation-JSON, Billing-Validation-HTTP-Contract, Billing-Restore-Coordinator/Mapping, Billing-Runtime-Restore-Applier, lokalen Crash-Report-Formatter/Inventory und Support-Diagnose-Export.
