...
- Entitlement-Domainlogik trennt App-Stufen von Kartenlizenzen; Play-Kauf/Restore fuehrt weiter nur ueber servervalidierte Entitlements.
...
- Feature-Access-Policy und Runtime-Gate ordnen Widgets und Kernfunktionen testbar `Free`, `Pro`, `Navigator` und `Fleet` zu; Premium-Widgets werden beim Hinzufuegen und beim Rendern bereits gespeicherter Cards geblockt, ohne Kartenlizenzen mit App-Abos zu vermischen.
- Play Billing Library ist als App-Subscription-/In-App-Gateway vorbereitet; Kauf-/Restore-Mapping gewaehrt Entitlements erst nach verifiziertem Kaufstatus und markiert unacknowledged Tokens.
...
- Keine vollstaendige Store-faehige Release- und Signing-Schiene; Play-Billing-Kauf/Restore ist vorhanden, aber Play-Console-Produkte, echte Kauf-Device-QA und produktiver Server-Validator sind noch offen.
...
Aktueller Implementierungsstand: Die Entitlement-Policy existiert als getestete Domainlogik fuer `Free`, `Pro`, `Navigator` und `Fleet`. Ein Billing-Katalog definiert aktive App-Abo-Produkt-IDs, ein aktives first-party Premium-Kartenpaket als Play-`INAPP` und inaktive externe Kartenlizenz-Platzhalter. Eine Feature-Access-Policy mappt Widgets/Funktionen auf die benoetigten App-Features; das Runtime-Gate blockiert Premium-Widgets beim Hinzufuegen, prueft bestehende Premium-Widgets beim Dashboard-Rendering erneut und zeigt bei Downgrade/Expiry eine klare gesperrte Card statt Premium-Funktion. Support-Diagnose-Share ist ebenfalls an `supportDiagnostics` gebunden: ohne Navigator/Fleet wird kein Bericht erzeugt und kein Sharesheet geoeffnet. Das Play-Billing-SDK ist angebunden und ein Gateway kann App-Abos sowie One-Time-In-App-Produkte wiederherstellen/acknowledgen. Der Menuepunkt `Abo & Karten` zeigt den lokalen Entitlement-Status, startet Play-Kaufdialoge fuer aktive Katalogprodukte, kann aktive Play-Kaeufe wiederherstellen und zeigt first-party Kartenpakete als `Nicht gekauft`, `Gekauft, Paketdatei fehlt`, `Installiert` oder `Lizenz abgelaufen`. Ein Play-Purchase-Adapter erzeugt daraus standardmaessig unverified Records; ein Restore-Coordinator erzeugt Backend-Validation-Requests, ein optionaler HTTP-Validation-Client nutzt `SEAFOX_BILLING_VALIDATION_URL`, und ein Runtime-Applier schreibt den `EntitlementSnapshot` nur nach vollstaendiger Verifikation in den App-State. Ohne Backend-Endpoint oder bei fehlenden Serverantworten wird nichts freigeschaltet und ein bestehender Snapshot nicht versehentlich ueberschrieben. Play-Console-Produkte/Preise, Trial-Regeln, produktiver Backend-Service, Kauf- und Restore-Device-QA, Premium-Pack-Downloadauslieferung und Runtime-Enforcement-Abdeckung weiterer Premium-Aktionen sind noch offen. Kartenanbieter werden nur ueber separate lizenzierte Provider-IDs freigeschaltet, nicht automatisch ueber eine App-Stufe.
