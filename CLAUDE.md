...
| Datei | Rolle |
|---|---|
| `ui/widgets/chart/ChartWidget.kt` | MapLibre-Compose-Integration, Lifecycle, Follow-Mode |
| `ui/widgets/chart/ChartStyle.kt` | Day/Night, Online/Offline-Fallback |
| `ui/widgets/chart/NauticalOverlay.kt` | S-57 ENC → 17 MapLibre-Layer |
| `ui/widgets/chart/AisOverlay.kt` | AIS-Ziele mit Bedrohungsklassifikation |
...
| `ui/widgets/chart/s57/S57Reader.kt` | S-57 Spatial/Feature-Extraktion |
| `ui/widgets/chart/s57/S57ToGeoJson.kt` | S-57 → GeoJSON-Konvertierung |
...
Prio 1 (MUSS — seetauglich machen):
  ├── S-57 ENC Polygon-Topologie fixen
  ├── SCAMIN-Filtering implementieren
  ├── CATALOG.031-Parser → positionsbasierte Zellauswahl
  ├── Multi-Scale-Handling (auto-load bei Zoom)
  ├── Navigationsvektoren (Kurs/Heading-Vektor, Prediktor, COG-Linie)
  └── OpenSeaMap Overlay (Raster-Tiles, trivial)

Prio 2 (SOLL — Offline-Navigation):
  ├── Waypoint-Navigation (Routen, Legs, XTE)
...
Immer im Repository-Root arbeiten und den Projekt-Wrapper `./gradlew` verwenden.
...
1. **S-57 Polygon-Topologie defekt** — Edge-Verkettung, Interior Rings, Winding Order
2. **Navigationsvektoren fehlen komplett** — Settings existieren, Rendering nicht (Heading, COG, Prediktor, Route, MOB)
3. **SCAMIN-Filtering fehlt** — zu viele Features, Performance-Probleme
4. **Keine positionsbasierte Zellauswahl** — CATALOG.031-Parser fehlt
5. **Kein Multi-Scale-Handling** — alle Scales gleichzeitig geladen
6. **Kein Bootssymbol auf der Karte** — Position nur als MapLibre-Kameraposition sichtbar
7. **Kein Waypoint/Routen-System** — keine Route-Planung, kein XTE, kein ETA
8. **Cold-Start Kartenperformance** — alle Features auf einmal geparst
9. **Nur S-57 und MBTiles** — andere Formate noch nicht unterstützt
10. **Kein Offline-Download-Manager** — User muss Dateien manuell kopieren
11. **Kein Track-Recording** — gefahrene Strecke wird nicht aufgezeichnet
12. **AIS-Overlay ohne Vektoren** — nur Punkte, keine COG-Vektoren oder CPA-Linien
