...
- Nach dem Runtime-Widget-Lock-Schnitt wurde gezielt mit gesetztem Android SDK `./gradlew --no-daemon testDebugUnitTest --tests com.seafox.nmea_dashboard.data.RuntimeEntitlementGateTest compileDebugKotlin` gruen ausgefuehrt.
- Nach demselben Schnitt wurde auch `./scripts/seafox-product-check.sh --ci --release-r8` mit lokal gesetztem Android SDK gruen ausgefuehrt; Compile, alle JVM-Tests, Lint und Release-R8/minify bestanden. `adb` fehlt weiterhin.
- Nach dem Support-Diagnose-Entitlement-Gate wurde erneut `./scripts/seafox-product-check.sh --ci --release-r8` mit lokal gesetztem Android SDK gruen ausgefuehrt; `adb` fehlt weiterhin.
...
- `RuntimeEntitlementGateTest`: Premium-Widget-Erstellung, bestehende Premium-Widget-Nutzung und Support-Diagnose-Featurezugriff werden gegen den persistierten App-Entitlement-Snapshot geprueft; Deny-Messages nennen erforderliche Stufe, Ablaufzustaende und trennen Kartenpakete von App-Features.
