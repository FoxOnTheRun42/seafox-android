...
- `ChartProviderRegistry` gates selectable providers: NOAA, S-57 and OpenSeaCharts are selectable/beta/available; C-Map is license-required and not selectable; S-63 is not implemented and not selectable. Persisted `SeaChartWidgetSettings` provider names are normalized through the same gate, so stored C-Map/S-63 values fall back to NOAA instead of silently becoming active.
...
- Offline/format breadth: MBTiles raster discovery exists; vector MBTiles/PBF, PMTiles, WMS/WMTS, BSB/KAP, GPX/KML, S-63 and C-Map are not production-ready provider implementations.
...
- Checked unit-test surface: `ChartProviderRegistryTest`, `SeaChartWidgetSettingsModuleTest`, `SafetyContourPolicyTest`, `HazardOverlayBuilderTest` and `GeoCalcTest` cover provider gating, persisted provider normalization, safety-depth policy, DEPARE/DEPCNT/SOUNDG shallow filtering and geometry math. Missing test coverage remains for the exact safety-contour contract feature shape and real rendered ENC screenshots.
- Did not run a full build for this wiki-only update. The latest CEO sync source reports `./scripts/seafox-product-check.sh --ci` passed on 2026-04-24, with emulator/device QA blocked because `adb` was unavailable.
...
- Where should concrete `ChartProvider` implementations live, and what is the migration order for S-57, OpenSeaMap raster, MBTiles raster and future vector-tile providers?
