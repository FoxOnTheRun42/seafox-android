...
- **Entitlements are not fully runtime-enforced:** `EntitlementPolicy` models `Free`, `Pro`, `Navigator` and `Fleet`, and separates app features from licensed chart providers. `PlayBillingClientGateway` and `BillingEntitlementMapper` prepare restore/acknowledge plus verified entitlement mapping, but purchase UI, backend receipt validation, trial rules and UI/action enforcement remain open.
- **Billing catalog is app-subscription-only for now:** `BillingCatalog` defines active app-subscription product ids for `Pro`, `Navigator` and `Fleet`. Chart-license products for C-Map and S-63 are inactive external placeholders and do not grant app tiers or chart provider licenses.
...
- `EntitlementSnapshot` contains an app tier, optional licensed chart provider ids and optional expiry.
...
- `BillingCatalog` normalizes product lookup by trim/lowercase and exposes `activeProducts()`, `tierForProductId()` and `chartProviderForProductId()`.
- `PlayBillingClientGateway` uses Google Play Billing for subscription restore and acknowledge paths.
